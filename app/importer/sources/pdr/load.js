...
if (require.main === module) {
  ...
  if (!output_file) throw new Error('Please pass filename to write links to as first argument');
...
function main (callback) {
  ...
  request(base_url + page_no + '/', function then (err, res, body){
    ...
    for_each.multi(5)(articles, function(article_url, next){

      console.log("...", ++called, "/",total, article_url);
    
      extract_article(article_url, function(err, title, content, html){ // third arg: meta
