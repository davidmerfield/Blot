...
module.exports = function ($, output_directory, callback) {
  ...
  eachEl($, 'resource', function(el, next){
    ...
    if (!file_name) file_name = (hash_id || size) + '.' + mime_type.slice(mime_type.indexOf('/') + 1);
    ...
    fs.outputFile(file_path, data, function(err){
      ...
      console.log('RESOURCE',  hash_id, timestamp, encoding, mime_type, size, file_name);
