<window class="{{#classes}}{{class}} {{/classes}}">
  ...
  <contents>
    {{#contents}}
    <item class="{{#nested}}{{nested}}-{{depth}}{{/nested}} {{open}}">{{#name}}<icon class="{{folder}} {{file}} {{extension}}"></icon> {{name}}{{/name}}{{^name}}&nbsp;{{/name}}</item>
    {{/contents}}
  </contents>
