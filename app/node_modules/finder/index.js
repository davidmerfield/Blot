...
var codeeditor_template = fs.readFileSync(__dirname + '/codeeditor.mustache', 'utf8');
...
function codeeditor(lang, code) {
  ...
  code = code.split('&amp;apos;').join("'");
  console.log(lang, code);
  return mustache.render(codeeditor_template, {code: code, title: 'Your post.txt'});
}
...
function exports (html, callback) {
  ...
  $('pre.code').each(function(i, el){
    ...
    var source = $(el).find('code').length ? $(el).find('code').first() : $(el);
    var code = source.html().trim();
    var lang = $(el).attr('class').split('code').join('').trim();
    var html = codeeditor(lang, code);
