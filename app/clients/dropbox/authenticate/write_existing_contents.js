...
function empty_dropbox_folder(client, folder, callback) {
  ...
  client
    .filesListFolder({ path: folder, include_deleted: false, recursive: false })
    .then(function(res) {
      async.eachSeries(
        res.entries,
        function(entry, next) {
          ...
          client
            .filesDelete({ path: entry.path_lower })
...
function write_existing_contents(
  blogID,
  dropbox_folder,
  access_token,
  callback
  ...
  function iterate_folder(folder_path, iterate_folder_callback) {
    ...
    fs.readdir(local_path(folder_path), function(err, contents) {
      async.eachSeries(
        contents,
        function(item_name, next) {
          debug("stating", local_path(join(folder_path, item_name)));

          fs.stat(local_path(join(folder_path, item_name)), function(
            err,
            stat
          ) {
            ...
            files_to_write[
              dropbox_path(join(folder_path, item_name))
            ] = local_path(join(folder_path, item_name));
            next();
          });
        },
