...
DropboxContentHasher.prototype.update = function (data, inputEncoding) {
  ...
  if (!Buffer.isBuffer(data)) {
    if (
      ...
      throw new Error(
        "Invalid 'inputEncoding': " + JSON.stringify(inputEncoding)
      );
    }
...
if (require.main === module) {
  main(process.argv[2], function (err, hash) {
    if (err) throw err;
    console.log(hash);
    process.exit();
  });
  // (async () => {
  //   var hash = await main(process.argv[2]);
  //   console.log(hash);
  //   process.exit();
  // })();
}
...
async function main(path, callback) {
  ...
  if (callback) {
    action(callback.bind(this, null), callback);
  } else {
