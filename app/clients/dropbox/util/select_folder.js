...
function list_candidate_folders(client, callback) {
  ...
  client
    .filesListFolder({ path: "", include_deleted: false, recursive: false })
    .then(callback.bind(this, null))
    .catch(callback);
}
...
select_folder.route("/").get(function(req, res, next) {
  ...
  var client = createClient(req.account.access_token);
    ...
    async.eachSeries(
      ...
      function(blogID, nextBlog) {
        ...
        database.get(blogID, function(err, other_account) {
          if (err || !other_account)
            return nextBlog(err || new Error("No other account"));

          contents.forEach(function(folder) {
            if (other_account.folder_id === folder.id) folder.in_use = true;
          });
...
select_folder.route("/").post(function(req, res, next) {
  ...
  if (typeof folder_id !== "string")
    return next(new Error("Please pass a valid folder"));
  ...
  get_accounts(req.user.blogs, function(err, accounts) {
    ...
    if (existing_folder_ids.indexOf(folder_id) > -1)
      return next(new Error("Already in use"));
