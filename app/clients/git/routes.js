...
dashboard.post('/create', function(req, res, next){
  ...
  fs.stat(blog_folder + '/.git', function(err, stat){
    ...
    if (stat && !err) {
      return next(new Error('There is already a git repository in your blogs folder, please remove it'));
    }
    ...
    database.refresh_token(req.blog.id, function(err){
      ...
      repos.create(req.blog.handle, function(err){
        ...
        .then(function(){
          ...
          return git_repo_in_blog_folder.commit(['-m', 'Removed placeholder file']);
        })
        .then(function(){
...
site.use('/end', auth.connect(auth.basic({
