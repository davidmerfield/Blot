...
try {fs.mkdirSync('backups');}catch(e){}
  ...
  forEach(services, function(name, url, next){
    ...
    request(options, function(err, res){
      ...
      if (err || !res || !res.statusCode)
        response.push(err || 'No response');

      db.get(name).push(response).write();
...
function render () {
  ...
  var statuses = db.get('site').value().slice();
  var percent = statuses.filter(function(a){return a[2] === undefined;}).length / statuses.length;
  ...
  INDEX = template.render({
    ...
    date: date, up: up, percent: percent, ago: ago
  });
}
