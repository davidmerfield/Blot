...
var breakPoints = ['<< more >>', '{{more}}', '<<more>>', '<!-- more -->', '<!- more ->', '<!— more —>'];
var charMap = {
  '<':'&lt;',
  '>':'&gt;',
  '-':'&#x2014;'
...
function makeTeaser (html) {
  ...
  if (!foundABreakPoint) {
    ...
    root.contents().each(function(){
      ...
      if (invisible.indexOf(this.name) > -1)
        return;
...
function isBreakPoint (node) {
  ...
  // Comment NODES
  // <!-- more --> and <!-- MORE --> etc...
  if (node.type === 'comment' && content === 'more')
    return true;
  ...
  if (node.type === 'text' && breakPoints.indexOf(content) > -1)
    return true;
}

function containsBreakPoint (node) {

  if (node.data === undefined || node.type !== 'text')
    return false;
  ...
  for (var i in breakPoints)
    if (content.indexOf(breakPoints[i]) > -1)
      return true;
...
function stripOtherText (node) {
  ...
  for (var x in breakPoints) {
    if (content.indexOf(breakPoints[x]) > -1) {
      ...
      dataWithoutBreakPoint = data.slice(0, i) + data.slice(i + breakPoints[x].length);
...
(function tests (){
  ...
  function test(html, expected, expectedNewHTML, expectedMore) {
      ...
      try {
        ...
        if (expectedMore)
          assert.deepEqual(expectedMore, teaser !== html);
