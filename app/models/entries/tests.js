...
const { dateStamp, metadata } = require("../entry/model");
...
describe("entries", function () {
    ...
    describe("getTotal", function () {
        it("should return the total number of entries for a blog", async function (done) {
        ...
        it("should return 0 if no entries exist for a blog", function (done) {
            Entries.getTotal(this.blog.id, function (err, total) {
                expect(err).toBeNull();
                expect(total).toBe(0);
                done();
    ...
    describe("getAllIDs", function () {
        it("should return all entry IDs for a blog", async function (done) {
        ...
        it("should return an empty array if no entries exist", function (done) {
            Entries.getAllIDs(this.blog.id, function (err, ids) {
                expect(err).toBeNull();
                expect(ids).toEqual([]);
                done();
    ...
    describe("getRecent", function () {
        it("should return the most recent entries with their indices", async function (done) {
            const key = `blog:${this.blog.id}:entries`;
            ...
            spyOn(Entry, "get").and.callFake((blogID, ids, callback) => {
                const entries = ids.map((id, index) => ({ id, dateStamp: Date.now() - index * 1000 }));
                callback(entries);
            });
        ...
        it("should return an empty array if there are no entries", function (done) {
            Entries.getRecent(this.blog.id, function (entries) {
