module.exports = function(server) {
  ...
  function loadPlugins (req, res, next) {

    var category = req.params.category || 'typography';
    var blog = req.blog;
    ...
    res.addPartials({sub_nav: 'dashboard/plugins/_nav'});
    ...
    res.addLocals({
      title: 'Blot - Plugins',
      plugins: plugins,
      tab: {plugins: 'selected'}
    });

    res.addPartials({
      yield: 'dashboard/plugins/form'
    });

    res.render('dashboard/_wrapper');
  }

  server.get('/plugins', auth.enforce, loadPlugins);
  server.get('/plugins/:category', auth.enforce, loadPlugins);

  /// Post files
  server.post('/plugins*', auth.enforce, bodyParser.urlencoded({extended:false}), function(request, response){
