...
function dateFromFileName (path) {
  ...
  for (var i = 0; i < tokens.length;i++) {
    ...
    if (parsed.isValid()) {
      ...
      fileName = path.slice(-tokens.slice(i + 3).join(' ').length - 1).trim();
      ...
      if (fileName === path.slice(path.lastIndexOf('.')))
        fileName = '';
...
function parseYear (token) {
  return token !== false &&
         ...
         parseInt(token); // this is returned
}
...
function within (num, min, max) {
  return num !== false &&
         ...
         num >= min &&
         num <= max &&
         num; // the last value is returned
}
...
function parseTime (token) {
  return token !== false &&
         ...
         token.length < 3 &&
         !isNaN(parseInt(token)) &&
         parseInt(token); // the last value is returned
}
...
(function tests () {
  ...
  function check (str, date, fileName) {
    ...
    try {
      ...
      if (res === false) console.log('No date parsed');
        ...
        console.log('DATE       parsed:', res.created, moment.utc(res.created).format('YYYY-M-D HH:mm:ss'));
        console.log('         expected:', date, moment.utc(date).format('YYYY-M-D HH:mm:ss'));
        console.log('----------------------------------------');
  ...
  // Hour as well
  check('2013-09-18-19.20.36.jpg', 1379532036000, '19.20.36.jpg');
  check('2013-01-31-00.00.1.jpg', 1359590401000, '00.00.1.jpg');
