...
describe("screenshot plugin", function () {
    ...
    it("respects rate limiting", async function () {
        ...
        const paths = Array.from({ length: requests }, (_, i) => `${__dirname}/data/screenshot_${i}.png`);
        ...
        console.log(requestTimes.map((t, i) => `Request ${i+1}: ${t}`).join("\n"));
        ...
        console.log(timeDiffs.map((diff, i) => `Diff between req ${i+1} and ${i+2}: ${diff}ms`).join("\n"));
        ...
        for (const p of paths) {
            ...
            if (hash !== expectedHash) {
                throw new Error(`Screenshot at ${p} does not match expected hash, please check the file.`);
            }
            fs.unlinkSync(p);
            }
        }
