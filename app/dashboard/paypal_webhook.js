...
const verifyWebhook = async (req, res, next) => {
  ...
  const certificate_url = req.headers["paypal-cert-url"];
  ...
  try {
    const certificate = await fetchCertificate(certificate_url);

    console.log("transmission_signature", transmission_signature);
...
const fetchCertificate = async certUrl => {
  ...
  console.log("fetching certificate", certUrl);
