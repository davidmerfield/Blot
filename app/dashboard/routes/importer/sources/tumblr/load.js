...
function get_posts (source_url, total_posts, callback) {
  ...
  for (var offset = 0; offset < total_posts ; offset += 20) {
    urls.push(string(URL_TEMPLATE + '&offset={{offset}}&limit={{limit}}').template({
      url: source_url,
      offset: offset,
      limit: 20,
      API_KEY: API_KEY,
  ...
  for_each(urls, function(url, next){
...
if (require.main === module) {
  ...
  if (!output_file) throw new Error('Please pass filename to write blog to as second argument');
