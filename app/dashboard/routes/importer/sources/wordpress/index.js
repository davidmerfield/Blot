...
if (require.main === module) {
  ...
  if (!outputDirectory || !sourceFiles.length) {
    console.log('Please pass XML export file(s) to convert and directory to output result:')
    return console.log('node index.js export.xml [other-export.xml] output-directory');
  }
    ...
    function(sourceFile, next) {
      fs.readFile(sourceFile, "utf-8", function(err, xml) {
        ...
        parseXML(xml, function(err, result) {
          ...
          console.log(colors.dim('Export Version'), result.rss.channel[0]['wp:wxr_version'][0]);
          ...
          async.eachOfSeries(result.rss.channel[0].item, function(item, index, done){
